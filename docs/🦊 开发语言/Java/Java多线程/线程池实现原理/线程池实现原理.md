...
![](https://ricear.com/media/202108/2021-08-03_153107.png)
...
2. `ThreadPoolExecutor` 的运行机制如下图所示：![图 2 ThreadPoolExecutor 运行流程](https://ricear.com/media/202108/2021-08-03_1615400.49959836104551203.png)
...
![图 3 线程池生命周期](https://ricear.com/media/202108/2021-08-03_1737060.37733254829039.png)
...
2. 当**试图通过 `execute()` 方法将一个**`Runnable` 任务添加到线程池中时，按照如下顺序来处理：
   ...
   2. 然后**检查运行线程数**，**决定接下来执行的流程**：
      ...
      ![](https://ricear.com/media/202108/2021-08-04_0947410.23325597268259834.png)
      
      ![11](https://ricear.com/media/202108/2021-08-05_0958330.09879545066325768.png)
      
      ![22](https://ricear.com/media/202108/2021-08-05_0958470.916503821148198.png)
...
4. **阻塞队列常用于生产者和消费者的场景**，**生产者是往队列里添加元素的线程**，**消费者是从队列里拿元素的线程**，**阻塞队列就是生产者存放元素的容器**，而**消费者也只是从容器里拿元素**，如下图所示：![图 5 阻塞队列](https://ricear.com/media/202108/2021-08-04_1035250.31094488351753713.png)
...
2. **线程需要从任务缓存模块中不断地取任务执行**，**帮助线程从阻塞队列中获取任务**，**实现线程管理模块和任务管理模块之间的通信**，**这部分策略由 `getTask` 方法实现**，其执行流程如下图所示：![图 6 获取任务流程图](https://ricear.com/media/202108/2021-08-04_1140410.08117444125024054.png)
...
2. Worker 这个工作线程，实现了 `Runnable` 接口，并持有一个线程 `thread`，一个初始化的任务 `firstTask`：
   ...
   ![图 7 Worker 执行任务](https://ricear.com/media/202108/2021-08-04_1514310.9596805483939324.png)
...
4. Worker 是通过继承 AQS，**使用 AQS 来实现独占锁这个功能**，**通过实现不可重入的特性去反映线程现在的执行状态**：
   ...
   3. 线程池在**执行 `shutdown()` 方法或 `tryTerminate()` 方法时会调用 `interruptIdleWorkers()` 方法来中断空闲的线程**，`interruptIdleWorkers()` 方法会使用 `tryLock()` 方法来判断线程池中的线程是否是空闲状态，如果线程是空闲状态，**则可以安全回收**。![图 8 线程池回收过程](https://ricear.com/media/202108/2021-08-04_1537570.5383356998637554.png)
...
3. `addWorker()` 方法的执行流程如下图所示：![图 9 申请线程执行流程图](https://ricear.com/media/202108/2021-08-04_1550510.26064429955688506.png)
...
2. **Worker 被创建出来以后**，就**会不断地进行轮询**，然后**获取任务去执行**，**核心线程可以无限等待获取任务**，**非核心线程要限时获取任务**，**当 Worker 无法获取到任务**，**也就是获取的任务为空时**，**循环会结束**，**Worker 会主动消除自身在线程池内的引用**。
   ...
   ![图 10 线程销毁流程](https://ricear.com/media/202108/2021-08-04_1600030.20263527686884708.png)
...
1. 在 Worker 类中的 `run()` 方法调用了 `runWorker()` 方法来执行任务，其执行过程如下：
   ...
   2. `getTask()`**方法从阻塞队列中获取任务**：
      ...
      ![图 11 执行任务流程](https://ricear.com/media/202108/2021-08-04_1624560.0730340580168457.png)
