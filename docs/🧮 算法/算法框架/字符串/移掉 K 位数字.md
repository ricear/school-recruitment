...
1. 这道题让我们**从一个字符串数字中删除 $k$ 个数字**，**使得剩下的数最小**，其中一个思路是：
   ...
   4. 具体过程如下：
      ...
      2. 开始的时候遍历到的元素是 1，由于没有左侧相邻元素，因此没办法丢弃。
         ...
         ![](https://ricear.com/media/202206/2022-06-09_161031_494584.png)
      3. 接下来遍历到的是 4，由于 4 比左侧相邻的 1 大，如果选择丢弃左侧的 1，那么会使得剩下的数字更大，因此我们仍然选择不丢弃。

         ![](https://ricear.com/media/202206/2022-06-09_161059_488014.png)

         ![](https://ricear.com/media/202206/2022-06-09_161254_964943.png)
      4. 接下来遍历到的元素是 3，由于 3 比左侧相邻的 4 小，如果选择丢弃左侧的 4，那么会使得剩下的数字更小，因此我们选择丢弃。

      ![](https://ricear.com/media/202206/2022-06-09_161341_107625.png)

      ![](https://ricear.com/media/202206/2022-06-09_161349_030894.png)

      ![](https://ricear.com/media/202206/2022-06-09_161401_918565.png)

      ![](https://ricear.com/media/202206/2022-06-09_161410_717517.png)
      6. 后面的思路类似，我们就不继续分析了。
   ...
   ![](https://ricear.com/media/202206/2022-06-09_161730_605002.gif)
...
2. 沿用上面的知识，我们要做的就是**计算每一个字符的 $k$**，具体的算法如下：
   ...
   4. **是否丢弃的标准和[上面题目](#2-问题分析)类似**，**如果栈中相邻的元素字典序更大**，**那么我们选择丢弃相邻的栈中的元素**。
      ...
      ![](https://ricear.com/media/202206/2022-06-09_174914_549301.gif)
...
3. 然而**在一个数组中取 $k$ 个数字**，**并保持最小**，我们已经会了，但是如果问题扩展到两个，会有什么变化呢，实际上，问题本质并没有发生变化：
   ...
   3. 以题目中的 $nums_1 = [3, 4, 5, 6] \space nums_2 = [9, 1, 2, 5, 8, 3] \space k = 5$ 为例：
      ...
      3. 如何将 $[6]$ 和 $[9, 5, 8, 3]$ 合并，使得数字尽可能大，并且保持相对位置不变呢，实际上这个过程有点类似[归并排序](https://ricear.com/project-21/doc-872/#7-%E5%BD%92%E5%B9%B6%E6%8E%92%E5%BA%8F)中的治，而上面我们分别计算 $nums_1$ 和 $nums_2$ 的最大数的过程类似[归并排序](https://ricear.com/project-21/doc-872/#7-%E5%BD%92%E5%B9%B6%E6%8E%92%E5%BA%8F)中的分。
         ...
         ![](https://ricear.com/media/202202/2022-02-07_2219140.1347819552753512.png)
