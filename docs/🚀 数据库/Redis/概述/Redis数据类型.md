...
3. 整个 Redis 的数据结构组织如下图所示：
   ...
   ![数据类型及其数据结构](https://notebook.grayson.top/media/202107/2021-07-13_0955310.35536856972697195.png)
...
2. 结合下面的实例对上面定义中每个参数的具体含义说明如下：
   ...
   ![](https://notebook.grayson.top/media/202107/2021-07-13_110621.png)
   ...
   2. **惰性空间释放**：
      ...
      4. 例如：
         ...
         2. 假如有一个 SDS 值 `s` 为 `XYXaYYbcXYX`，如下图所示：
            ...
            ![](https://notebook.grayson.top/media/202107/2021-07-13_151706.png)
         3. 当执行完 `sdstrim(s, "XY")`（移除 SDS 字符串中的所有 `X` 和 `Y`）命令后，SDS 被修改为 `abc`，如下图所示：

            ![](https://notebook.grayson.top/media/202107/2021-07-13_152155.png)
         ...
         5. 例如，现在对 `s` 执行 `sdscat(s, " Redis")`，执行完成之后，`s` 被修改为 `abc Redis`，如下图所示：
            ...
            ![](https://notebook.grayson.top/media/202107/2021-07-13_152539.png)
...
2. 多个 `listNode` 可以通过 `prev` 和 `next` 指针组成**双链表**，如下图所示：
   ...
   ![](https://notebook.grayson.top/media/202107/2021-07-13_163156.png)
...
3. `list` 可以**和 `listNode` 结合组成链表**，如下图所示：
   ...
   ![](https://notebook.grayson.top/media/202107/2021-07-13_165739.png)
4. Redis 的链表具有如下特性：
...
3. 压缩列表的结构如下图所示：
   ...
   ![](https://notebook.grayson.top/media/202107/2021-07-14_101549.png)
...
4. 压缩列表的示例如下：
   ...
   ![](https://notebook.grayson.top/media/202107/2021-07-14_110450.png)
...
2. 哈希表的示例如下图所示：
   ...
   ![](https://notebook.grayson.top/media/202107/2021-07-14_142200.png)
...
1. 哈希表节点**使用 `dictEntry` 结构表示**，**每个 `dictEntry` 结构都保存着一个键值对**：
   ...
   3. `next`：表示**指向下个哈希节点的指针**，**这个指针可以将多个哈希值相同的键值对连接在一起**，以此来**解决键冲突的问题**：
      1. 如下图所示就是将两个索引值相同的键 `k1` 和 `k0` 连接在一起：
         ...
         ![](https://notebook.grayson.top/media/202107/2021-07-14_143147.png)
...
2. 下图展示了一个普通状态下（没有进行 `rehash`）的字典：
   ...
   ![](https://notebook.grayson.top/media/202107/2021-07-14_144923.png)
...
3. 具体的 `rehash` 例子如下：
   ...
   ![](https://notebook.grayson.top/media/202107/2021-07-14_155443.png)

   ![](https://notebook.grayson.top/media/202107/2021-07-14_155513.png)

   ![](https://notebook.grayson.top/media/202107/2021-07-14_155603.png)

   ![](https://notebook.grayson.top/media/202107/2021-07-14_155627.png)
...
6. 具体的渐进式 `rehash` 例子如下：
   ...
   ![](https://notebook.grayson.top/media/202107/2021-07-14_162213.png)

   ![](https://notebook.grayson.top/media/202107/2021-07-14_162302.png)

   ![](https://notebook.grayson.top/media/202107/2021-07-14_162650.png)

   ![](https://notebook.grayson.top/media/202107/2021-07-14_162757.png)

   ![](https://notebook.grayson.top/media/202107/2021-07-14_162843.png)

   ![](https://notebook.grayson.top/media/202107/2021-07-14_162933.png)
...
1. `ZSet` 底层有两种实现，分别是**压缩列表**和**跳表**：![](https://notebook.grayson.top/media/202107/2021-07-14_172414.png)
   1. 当一个有序集合包含的**元素数量不多**，并且有序集合中**元素的成员的长度不大**时，Redis 就会**使用压缩列表作为有序集合键的底层实现**：
      ...
      3. 例如：
         ...
         2. 元素在压缩列表中的保存方式如下图所示：
            ...
            ![](https://notebook.grayson.top/media/202107/2021-07-14_172517.png)
...
2. 跳表的结构如下图所示：
   ...
   ![](https://notebook.grayson.top/media/202107/2021-07-26_112537.png)
...
2. **层**：
   ...
   3. 下图分别展示了三个高度为 1 层、3 层和 5 层的节点：
      ...
      ![](https://notebook.grayson.top/media/202107/2021-07-26_144034.png)
3. **前进指针**：

   1. **每个层都有一个指向表尾方向的前进指针**，**用于从表头向表尾方向访问节点**，下图用虚线表示出了程序从表头向表尾方向，遍历跳表中所有节点的路径：![](https://notebook.grayson.top/media/202107/2021-07-26_144407.png)
...
4. **跨度**：
   ...
   2. **跨度主要用来计算排位**（Rank）**的**，**在查找某个节点的过程中**，**将沿途访问过的所有层的跨度累计起来**，**得到的结果就是目标节点在跳跃表中的排位**，例如：
      1. 下图用虚线标记了在跳跃表中查找分值为 3.0、成员对象为 $o3$ 的节点时沿途经历的层，并且层的跨度为 3，所以目标节点在跳跃表中的排位为 3。
         ...
         ![](https://notebook.grayson.top/media/202107/2021-07-26_145948.png)
      2. 下图用虚线标记了在跳跃表中查找分值为 2.0、成员对象为 $o2$ 的节点时沿途经历的层，在查找节点的过程中，程序经过了两个跨度为 1 的节点，因此可以计算出，目标节点在跳跃表中的排位为 2。

         ![](https://notebook.grayson.top/media/202107/2021-07-26_150258.png)
5. **后退指针**：
   ...
   2. 下图用虚线展示了如何**从表尾向表头遍历跳表中的所有节点**，**程序首先通过跳表的 `tail` 指针访问表尾节点**，**然后通过后退指针访问倒数第二个节点**，**之后再沿着后退指针访问倒数第三个节点**，**再之后遇到指向 `NULL` 的后退指针**，**于是访问结束**。
      ...
      ![](https://notebook.grayson.top/media/202107/2021-07-26_155255.png)
6. **分值和成员**：
   ...
   4. 如下图所示，三个跳表节点都保存了相同的分值 10086.0，保存成员对象 $o1$ 的节点排在保存成员对象 $o2$ 和 $o3$ 节点之前，而保存成员对象 $o2$ 的节点又排在保存成员对象 $o3$ 的节点之前，由此可见，$o1$、$o2$、$o3$ 三个成员对象在字典中的排序为 $o1 \lt o2 \lt o3$。
      ...
      ![](https://notebook.grayson.top/media/202107/2021-07-26_160619.png)

###### 2.5.4.2.2 跳表

1. **仅靠多个跳表节点就可以组成一个跳表**，如下图所示：

   ![](https://notebook.grayson.top/media/202107/2021-07-26_160844.png)
2. **但通过使用一个 `zskiplist` 结构来持有这些节点**，**程序可以更方便地对整个跳表进行处理**，比如**快速访问跳表的表头结点和表尾节点**，或者**快速地获取跳表节点的数量**（即跳表的长度）**等信息**，如下图所示：

   ![](https://notebook.grayson.top/media/202107/2021-07-26_161226.png)
3. `zskiplist` 的结构定义如下：
