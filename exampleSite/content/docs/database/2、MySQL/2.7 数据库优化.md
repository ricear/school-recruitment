...
1. **操作系统**：
   1. **增加TCP支持的队列数**（网络方面的配置，要修改 `/etc/sysctl.conf`）：
      ...
      ```
2. **MySQL配置文件优化**：
   1. `innodb_buffer_pool_size`：配置**缓冲池大小**，如果数据库中**只有 `innodb`表**，则**推荐配置量为总内存的75%**。
   ...
   5. `innodb_read_io_threads`：配置**读的IO进程数**，**默认为4**。
   6. `innodb_write_io_threads`：配置**写的IO进程数**，**默认为4**。
